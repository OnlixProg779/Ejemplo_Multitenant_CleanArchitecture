...
namespace Multitenant.Application.CQRS.Queries.Products.GetPaginParams.Resources
{
    public class ProductSpecificationParams
    {
        ...
        public List<Expression<Func<Domain.Bussines.Products, bool>>> GetStandarCriteria(ProductPaginParams standarBaseQuery)
        {
            ...
            {
                list.Add((Domain.Bussines.Products x) => x.Active == standarBaseQuery.Active);
            }

            if (!string.IsNullOrWhiteSpace(standarBaseQuery.CreatedBy))
            {
                list.Add((Domain.Bussines.Products x) => x.CreatedBy.ToLower().Contains(standarBaseQuery.CreatedBy.ToLower()));
            }

            if (!string.IsNullOrWhiteSpace(standarBaseQuery.LastModifiedBy))
            {
                list.Add((Domain.Bussines.Products x) => x.LastModifiedBy.ToLower().Contains(standarBaseQuery.LastModifiedBy.ToLower()));
            }
            ...
            {
                list.Add((Domain.Bussines.Products a) => a.CreatedDate >= standarBaseQuery.CreatedDateFrom && a.CreatedDate < standarBaseQuery.CreatedDateTo.Value.AddDays(1.0));
            }

            if ((standarBaseQuery.CreatedDateFrom.HasValue || Convert.ToString(standarBaseQuery.CreatedDateFrom) != "") && (!standarBaseQuery.CreatedDateTo.HasValue || Convert.ToString(standarBaseQuery.CreatedDateTo) == ""))
            {
                list.Add((Domain.Bussines.Products a) => a.CreatedDate >= standarBaseQuery.CreatedDateFrom && a.CreatedDate < DateTime.Now.AddDays(1.0));
            }

            if ((!standarBaseQuery.CreatedDateFrom.HasValue || Convert.ToString(standarBaseQuery.CreatedDateFrom) == "") && (standarBaseQuery.CreatedDateTo.HasValue || Convert.ToString(standarBaseQuery.CreatedDateTo) != ""))
            {
                list.Add((Domain.Bussines.Products a) => a.CreatedDate >= standarBaseQuery.CreatedDateTo && a.CreatedDate <= standarBaseQuery.CreatedDateTo.Value.AddDays(1.0));
            }
            ...
            {
                list.Add((Domain.Bussines.Products a) => a.LastModifiedDate >= standarBaseQuery.LastModifiedDateFrom && a.LastModifiedDate < standarBaseQuery.LastModifiedDateTo.Value.AddDays(1.0));
            }

            if ((standarBaseQuery.LastModifiedDateFrom.HasValue || Convert.ToString(standarBaseQuery.LastModifiedDateFrom) != "") && (!standarBaseQuery.LastModifiedDateTo.HasValue || Convert.ToString(standarBaseQuery.LastModifiedDateTo) == ""))
            {
                list.Add((Domain.Bussines.Products a) => a.LastModifiedDate >= standarBaseQuery.LastModifiedDateFrom && a.LastModifiedDate < DateTime.Now.AddDays(1.0));
            }

            if ((!standarBaseQuery.LastModifiedDateFrom.HasValue || Convert.ToString(standarBaseQuery.LastModifiedDateFrom) == "") && (standarBaseQuery.LastModifiedDateTo.HasValue || Convert.ToString(standarBaseQuery.LastModifiedDateTo) != ""))
            {
                list.Add((Domain.Bussines.Products a) => a.LastModifiedDate >= standarBaseQuery.LastModifiedDateTo && a.LastModifiedDate <= standarBaseQuery.LastModifiedDateTo.Value.AddDays(1.0));
            }
